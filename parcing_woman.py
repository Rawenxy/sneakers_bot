...
dbname = get_database()
collection_name = dbname["shoes_w"]

base_url = 'https://fireboxclub.com'
url = 'https://fireboxclub.com/catalogue/sale990?show=1&sGen=2'
...
def make_woman_db():
    ...
    for count in range(1, int(find_lats_pages)+1):
        ...
        for item in find_objects:
            ...
            collection_name.replace_one({'item_href': shoes_w['item_href']}, shoes_w, upsert=True)
...
def clear_derectory_data_g():
    try:
        ...
        print('Не получуилось удалить файлы')
